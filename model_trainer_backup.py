...
def evaluate_model(model, X_test, y_test, le_path='label_encoder.pkl', feat_path='feature_names.pkl'):
    ...
    for i in range(0, len(X_test), BATCH_SIZE):
        ...
        y_prob.extend(model.predict_proba(batch))
    ...
    # Calculate top3 and top5
    # y_prob is shape (n_samples, n_classes)
    # argsort returns indices of sorted properties, so [:, -3:] are the top 3
    # Note: we need to handle this efficiently without massive memory spikes
    top3_correct = 0
    top5_correct = 0
    
    for i in range(len(y_test)):
        # To save memory, taking argsort on a single row
